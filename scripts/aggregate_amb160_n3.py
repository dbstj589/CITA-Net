"""n=3 aggregation for the amb160 slots160 main run (seeds 19521006/07/08).
...
a df=2 paired t-test (closed-form; scipy not required), re-computes the gate
distribution per seed for consistency, writes CSV+MD and a bar chart.
Low power (n=3) is stated explicitly; sign consistency + dev/test agreement drive
...
def student_t_cdf_df2(t):
    # exact CDF of Student's t with df=2
    return 0.5 + t / (2.0 * math.sqrt(2.0) * math.sqrt(1.0 + t * t / 2.0))
...
def paired_t(diffs):
    ...
    se = sd / math.sqrt(n)
    t = md / se
    p = 2.0 * (1.0 - student_t_cdf_df2(abs(t)))
...
with open(OUT / "n3_metrics_raw.csv", "w", newline="") as f:
...
with open(OUT / "n3_summary.csv", "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["model", "split", "metric", "mean", "std",
                "seed19521006", "seed19521007", "seed19521008"])
...
md = []
md.append("# amb160 main run — n=3 (seeds 19521006/07/08)\n")
md.append("Data `data/hard_ambiguity/amb160` (~109 identities/sector). Decoder `num_slots=160`. "
          "epochs=40, full-sectors, decode_full scoring. Identical settings across all 3 seeds.")
md.append("**n=3 → low statistical power; paired t-test df=2 is reported but interpretation leans on "
...
for (A, B, tag) in [("m3_full", "no_motion", "reversal: does motion help under ambiguity?"),
    ...
    md.append("| split | per-seed ΔF1 (06/07/08) | mean Δ | sign-consistent | t(df=2) | p(2-tail) |")
    md.append("|---|---|---|---|---|---|")
    for sp in SPLITS:
...
fig.suptitle("Motion term reverses sign: hurts on easy (frozen), helps on hard (amb160)   [amb160 n=3 mean±std]")
fig.tight_layout()
fig.savefig(OUT / "n3_f1_reversal.png", dpi=140)
print("WROTE:")
for p in ["n3_metrics_raw.csv", "n3_summary.csv", "n3_report.md", "n3_f1_reversal.png"]:
    print("  ", OUT / p)
print("\n----- REPORT -----\n")
print((OUT / "n3_report.md").read_text(encoding="utf-8"))
