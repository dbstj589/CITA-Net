...
def main():
    ...
    ap.add_argument("--gt", default="data/hill395_world_gt_full10d")
    ap.add_argument("--frames", type=int, default=1440)
    ap.add_argument("--overview-frames", type=int, default=360)
    ...
    # real terrain peak (local coords) — renderer marks the actual crest here,
    # since per-sector landmark labels do NOT coincide with the global peak.
    _pi, _pj = np.unravel_index(int(np.argmax(elev)), elev.shape)
    terrain_peak = {"x": round(_pj * res, 1), "y": round(_pi * res, 1),
                    "elevation": round(float(elev[_pi, _pj]), 1)}
    ...
    peak_day = peak_t / 86400.0
    stats = {"concurrent_alive": {"peak": peak, "peak_time_s": peak_t,
                                  "peak_time_dhm": f"day {int(peak_day)} {int((peak_t%86400)//3600):02d}h",
                                  "median": med, "mean": round(mean, 1), "series_step_s": dt,
                                  "series": alive_series},
             ...
             "coord_exceeds_extent": bool(bbox["xmin"] < 0 or bbox["xmax"] > map_extent["xmax"]
                                          or bbox["ymin"] < 0 or bbox["ymax"] > map_extent["ymax"]),
             "terrain_peak_local": terrain_peak,
    ...
    meta = {"source_gt": str(gt), "global_seed": manifest.get("global_seed"),
            "time": {"epoch": epoch, "window_seconds": W, "window_days": round(W / 86400, 2),
                     "dt_seconds": dt, "frames": F,
                     "resample": (f"per-entity resample of the GT samples -> {F} frames within each "
                                  "entity's active window: linear interp of (x,y); state/alive via floor "
                                  "(nearest-earlier) sample; rows only for active frames. GT has more raw "
                                  f"sample instants (~4.6k) than frames ({F}) so this is a DOWNSAMPLE "
                                  "(interp picks the representative value at each frame time).")},
            "map": {"unit": "meters", "base_offset": {"easting": off_e, "northing": off_n,
                    ...
                    "coord_exceeds_extent_note": ("entity coords EXCEED the terrain extent (sector offsets "
                            "> 2km pitch); use trajectory_bbox for the canvas. Terrain lookups clamp to edges."),
                    "terrain_peak_local": terrain_peak,
    ...
    print(f"  concurrent alive: peak={peak} @ t={peak_t}s (day {peak_t/86400:.1f})  median={med}  mean={mean:.1f}")
    print(f"  terrain peak (local): x={terrain_peak['x']} y={terrain_peak['y']} elev={terrain_peak['elevation']}m")
    print(f"  coord exceeds terrain extent: {stats['coord_exceeds_extent']}  (bbox {bbox})")
