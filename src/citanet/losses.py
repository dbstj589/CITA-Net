...
def transition_loss(out: ModelOutput, feats: ScenarioFeatures,
    ...
    """BCE on CTA transition scores for within-identity positives vs hard
    negatives (transition_level.json). Pairs absent from the candidate set are
    skipped (blocking already filtered them)."""
    tl = json.loads((Path(scenario_dir) / "labels" / "transition_level.json").read_text(encoding="utf-8"))
    ...
    if not logits:
        return torch.zeros((), requires_grad=True)
    return F.binary_cross_entropy_with_logits(torch.stack(logits),
                                              torch.tensor(labels))
...
def assign_loss(out: ModelOutput, feats: ScenarioFeatures,
    ...
    obs_level = json.loads((sdir / "observation_level.json").read_text(encoding="utf-8")
                           ) if (sdir / "observation_level.json").exists() else \
        json.loads((sdir / "labels" / "observation_level.json").read_text(encoding="utf-8"))
    dangling = json.loads((sdir / "labels" / "dangling.json").read_text(encoding="utf-8"))
    dang_obs = set(dangling["dangling_observations"])

    obs_to_gold = {a["obs_id"]: a["gold_identity_id"] for a in obs_level["assignment"]}
